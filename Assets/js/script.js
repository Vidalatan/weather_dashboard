...
function getCurrentCoordsWeather(coords, event=null, city=null) {
    ...
    .then(response => {
        ...
        $("#display-wind_speed").text(data.current.wind_speed)
        ...
        for (index in data.daily) {
            ...
            .append($("<div>").addClass("row").text("Temp: "+indexed_day.temp.day+"℉"))
            .append($("<div>").addClass("row").text("Wind: "+indexed_day.wind_speed+" MPH"))
            .append($("<div>").addClass("row").text("Humidity: "+indexed_day.humidity+"%"))                
            ))
        }
    })
}
